...
async def _call(tool_name: str, args: dict) -> dict:
    ...
    # call_tool returns Sequence[ContentBlock]; first block is TextContent with JSON
...
class TestResolveAlias:
    ...
    async def test_resolved(self, client):
        entity = await _post(client, "/entities", {
            "type": "FEATURE", "canonical_name": "사용자 검색"
        })
        await _post(client, f"/entities/{entity['id']}/aliases", {
    ...
    async def test_ambiguous(self, client):
        e1 = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "검색 A"})
        e2 = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "검색 B"})
        for eid in [e1["id"], e2["id"]]:
            await _post(client, f"/entities/{eid}/aliases", {"locale": "ko", "alias": "검색"})
...
class TestGetEntity:
    ...
    async def test_deprecated_warning(self, client):
        e1 = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "구 기능"})
        e2 = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "신 기능"})
        await client.patch(f"/entities/{e1['id']}", json={
...
class TestSearchEntities:
    async def test_alias_exact(self, client):
        entity = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "기능 X"})
        await _post(client, f"/entities/{entity['id']}/aliases", {
    ...
    async def test_canonical_name_partial(self, client):
        entity = await _post(client, "/entities", {"type": "UI_AREA", "canonical_name": "사용자 목록 화면"})
    ...
    async def test_type_filter(self, client):
        feat = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "공통 기능"})
        area = await _post(client, "/entities", {"type": "UI_AREA", "canonical_name": "공통 영역"})
...
class TestGetContextBundle:
    async def test_bundle_root_only(self, client):
        entity = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "번들 루트"})
        await _post(client, f"/entities/{entity['id']}/contexts", {
    ...
    async def test_bundle_with_related(self, client):
        root = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "루트 기능"})
        child = await _post(client, "/entities", {"type": "INFRA_UNIT", "canonical_name": "연관 인프라"})
        await _post(client, "/relations", {
    ...
    async def test_bundle_deprecated_warning(self, client):
        old = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "구 기능"})
        new = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "신 기능"})
...
class TestValidateReferences:
    async def test_all_resolved(self, client):
        entity = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "검증 기능"})
        await _post(client, f"/entities/{entity['id']}/aliases", {
    ...
    async def test_ambiguous_alias(self, client):
        e1 = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "중복 기능 A"})
        e2 = await _post(client, "/entities", {"type": "FEATURE", "canonical_name": "중복 기능 B"})
        for eid in [e1["id"], e2["id"]]:
            await _post(client, f"/entities/{eid}/aliases", {"locale": "ko", "alias": "중복alias"})
