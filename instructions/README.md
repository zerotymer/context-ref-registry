...
## Phase 1.6 — Frontend BFF 전환

**목표**: 브라우저에서 backend(`:8000`)를 직접 호출하지 않고 Next.js BFF route를 경유한다.

| 순서 | UUID | 지침 파일 | 상태 |
|------|------|-----------|------|
| 1.6 | 5fa5df8b | frontend_bff_proxy.md | `pending` |

### 포함 범위

```
1.6-1   Next.js route handler 기반 BFF proxy 추가
1.6-2   frontend API client를 /api/backend 상대 경로 호출로 전환
1.6-3   NEXT_PUBLIC_API_URL 제거, BACKEND_API_URL server-only 변수 도입
1.6-4   Docker Compose frontend 환경 변수 정리
1.6-5   CORS/보안 정책 후속 정리
```
...
| 36ab4117-e0cc-4a30-813e-129f0835b540 | tag_and_history.md | 1.7 | `pending` |
| 5fa5df8b-61e9-4da7-86d5-9802e748c405 | frontend_bff_proxy.md | 1.6 | `pending` |
