...
- [x] **C-1 — 스트리밍 프록시 핸들러**
  - `frontend/src/lib/api/mcp-gateway.ts` `proxyMcpToBackend` + 라우트
    `frontend/src/app/api/v1/mcp/[[...path]]/route.ts` (GET/POST/DELETE). 정적
    `mcp` 세그먼트가 기존 `[...path]` REST 게이트웨이보다 우선 매칭.
  - 요청/응답 **바디 스트림 pipe** (`request.body` + `duplex:"half"`, 응답은
    `backendRes.body` 그대로). 버퍼링 없음. SSE `content-type` 보존.
  - 백엔드 타깃은 `/mcp/` (트레일링 슬래시) — `/mcp`→`/mcp/` 307 우회(스트림
    바디는 재전송 불가). `redirect:"manual"`.
  - 헤더 화이트리스트: `authorization`·`x-api-key`·`content-type`·`accept`·
    `mcp-session-id`·`mcp-protocol-version`·`last-event-id` passthrough, 쿠키 차단.
    응답은 `content-type`·`cache-control`·`mcp-session-id`·`mcp-protocol-version`.
  - `runtime = "nodejs"`, `dynamic = "force-dynamic"`.
- [x] **C-2 — 백엔드 노출 (회수 보류)**
  - 결정: `:8000` 공개 유지(로컬 dev /docs·REST). compose 주석에 권장 외부
    진입점이 BFF(`:3000/api/v1/mcp`)임을 명시. 포트 회수는 배포 단계로 이연.
    (프론트→백엔드는 docker 네트워크 `BACKEND_API_URL=http://api:8000`로 동작.)
- [x] **C-3 — 테스트**
  - `frontend/src/lib/api/mcp-gateway.test.ts` (11 cases) — 타깃 경로(`/mcp/`)·
    `redirect:manual`·auth/MCP 세션 헤더 passthrough·쿠키 차단·요청 스트림
    (`duplex:half`, 무버퍼)·응답 SSE 스트림 pipe·세션 헤더·401 passthrough.
    프론트 **27 passed** (16 기존 + 11 신규).
  - 실서버 e2e 수동 검증: api 컨테이너 + `next dev`(BACKEND_API_URL=:8000) +
    MCP 클라이언트로 `:3000/api/v1/mcp` 붙어 initialize→list_tools(7종)→
    search_entities/get_entity 성공. 키 없음/오류/쿠키-only → 401 확인.
- [x] **C-4 — 문서 동기화**
  - `CLAUDE.md`(현황·아키텍처·구조·지침표·테스트 수), `AGENTS.md`,
    `instructions/README.md`, `docker-compose.yml` 주석 — 에이전트 MCP 진입점이
    `/api/v1/mcp` 임을 명시.

### 단계 C 완료 기준 (DoD)

- MCP 클라이언트가 `http://host:3000/api/v1/mcp` (API Key 헤더)로 붙어 tool 호출
  성공 — **실서버 e2e로 검증** (initialize → list_tools 7종 → search/get_entity).
- 프론트 테스트 그린 (**27 passed**).
- (보류) 백엔드 `:8000` 외부 직접 접근 차단은 배포 단계로 이연 — 결정상 dev
  편의를 위해 `:8000` 공개 유지, 권장 진입점만 BFF로 문서화.
