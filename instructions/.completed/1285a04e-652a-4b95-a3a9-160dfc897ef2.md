...
- [x] `GET /admin/users`
- [x] `POST /admin/users`
- [x] `PATCH /admin/users/{user_id}`
- [x] `POST /admin/users/{user_id}/reset-password`
- [x] 사용자 비활성화 API
- [x] 관리자 발급 계정 생성 화면/API에서 임의 가입 경로가 없도록 검증
...
- [x] `PATCH /admin/projects/{project_id}`
- [x] `GET /admin/projects/{project_id}/members`
- [x] `POST /admin/projects/{project_id}/members`
- [x] `PATCH /admin/projects/{project_id}/members/{user_id}`
- [x] `DELETE /admin/projects/{project_id}/members/{user_id}`
- [x] 프로젝트 관리자 임명/해제 API는 `admin` 전용으로 제한
...
- [x] Frontend BFF에서 인증 cookie/session 전달 구조 반영
- [x] 관리자 메뉴는 권한별로 노출 제어
- [x] API 에러 코드를 사용자 행동 단위 메시지로 매핑
- [x] 프로젝트 선택 필터를 entity 목록/상세/수정 화면에 연결
