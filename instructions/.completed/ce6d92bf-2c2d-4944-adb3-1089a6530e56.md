...
- [x] FastAPI middleware로 요청 로깅 (request_id, endpoint, result_status, latency_ms)
- [x] structlog으로 JSON 구조화 로그 출력
- [x] `GET /health` 엔드포인트 (DB 연결 상태 포함)
- [x] Docker Compose `healthcheck` 설정 (api, postgres)
